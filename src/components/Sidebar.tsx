    ...
    return (
        <>
            ...
            <aside className={clsx(
                ...
                <div className="p-5 flex-shrink-0 flex items-center justify-between">
                    <LogoCombined />
                ...
                <nav className="flex-1 space-y-1 px-3 py-2 overflow-y-auto no-scrollbar">
                    {navItems.map((item) => (
                        ...
                        >
                            <item.icon size={22} className={clsx("transition-colors md:w-[18px] md:h-[18px]", ({ isActive }: any) => isActive ? "text-primary" : "group-hover:text-primary")} />
                            <span className="text-base md:text-sm">{item.label}</span>
                        </NavLink>
                    ))}
                </nav>

                <div className="p-3 mx-2 mb-2 border-t border-zinc-100 dark:border-zinc-800 space-y-2 flex-shrink-0">
                    ...
                    <button
                        ...
                        className="w-full flex items-center gap-4 px-5 py-3.5 md:py-2.5 rounded-2xl md:rounded-xl text-zinc-500 dark:text-zinc-400 md:hover:bg-zinc-50 md:dark:hover:bg-zinc-900 md:hover:text-primary transition-all font-bold md:font-medium active:bg-zinc-100 dark:active:bg-zinc-900"
                    >
